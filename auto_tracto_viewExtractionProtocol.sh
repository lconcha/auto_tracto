...
echolor yellow "There are $n exclusions"
if [ $n -eq 0 ]; then  exclusions=""
else
  for excl in ${protocolsFolder}/protocols/${structure}/exclude*
  do
    exclusions="$exclusions -roi.load $excl -roi.colour 1,0,0 -roi.opacity 0.5"
  done
fi
...
echolor yellow "There are $n seeds"
if [ $n -eq 0 ]; then  seeds=""
else
  for seed in ${protocolsFolder}/protocols/${structure}/seed*
  do
    seeds="$seeds -roi.load $seed -roi.colour 0,1,0 -roi.opacity 0.5"
    n=$(( $n +1 ))
  done
fi
...
echolor yellow "There are $n targets"
if [ $n -eq 0 ]; then  targets=""
else
  for target in ${protocolsFolder}/protocols/${structure}/target*
  do
    targets="$targets -roi.load $target -roi.colour 0,1,0 -roi.opacity 0.5"
    n=$(( $n +1 ))
  done
fi
...
echolor yellow "There are $n stops"
if [ $n -eq 0 ]; then  stops=""
else
  for stop in ${protocolsFolder}/protocols/${structure}/stop*
  do
    stops="$stops -roi.load $stop -roi.colour 0,0,1 -roi.opacity 0.5"
    n=$(( $n +1 ))
  done
fi
